 ...
 *
 * $Id: sqlite3odbc.c,v 1.74 2007/10/11 08:01:21 chw Exp chw $
 *
...
#define MAX_PARAMS_FOR_MPRINTF 32
#endif
...
tbldone:
    ...
    if (rc != SQLITE_OK && rc == sqlite3_errcode(d->sqlite) && errp) {
...
#endif
    rc = sqlite3_open_v2(name, &d->sqlite, flags, NULL);
#if defined(WINTERFACE) || defined(_WIN32)
    if (uname != name) {
	uc_free(uname);
    }
...
s3stmt_start(STMT *s, char **params)
{
	...
	sql = sqlite3_mprintf(s->query,
			      ...
			      params[30], params[31]);
#else
	sql = sqlite3_vmprintf((char *) s->query, (char *) params);
#endif
	if (!sql) {
	    return nomem(s);
	}
	...
	s3stmt = NULL;
#if defined(HAVE_SQLITE3PREPAREV2) && HAVE_SQLITE3PREPAREV2
	rc = sqlite3_prepare_v2(d->sqlite, sql, -1, &s3stmt, &endp);
#else
...
substparam(STMT *s, char **sqlp, int pnum, char **outp)
{
	...
	*pp++ = '\'';
	for (i = 0; i < len; i++) {
	    *pp++ = xdigits[(((char *) p->param)[i] >> 4) & 0xF];
	    *pp++ = xdigits[((char *) p->param)[i] & 0xF];
	}
	*pp++ = '\'';
	*pp++ = '\0';
	freep(&p->parbuf);
	p->parbuf = p->param = param;
	p->len = len * 2 + 4 - 1;
	p->max = p->len;
	p->need = -1;
	if (sql) {
    ...
    case SQL_C_SBIGINT:
#ifdef _WIN32
	sprintf(p->strbuf, "%I64d", *(SQLBIGINT *) p->param);
#else
	sprintf(p->strbuf, "%lld", *(SQLBIGINT *) p->param);
#endif
	goto bind;
    case SQL_C_UBIGINT:
#ifdef _WIN32
	sprintf(p->strbuf, "%I64u", *(SQLUBIGINT *) p->param);
#else
	sprintf(p->strbuf, "%llu", *(SQLUBIGINT *) p->param);
#endif
	goto bind;
#endif
    case SQL_C_FLOAT:
		   ...
		   SQLCHAR *table, SQLSMALLINT tableLen)
{
    ...
    HSTMT_LOCK(stmt);
    ret = mkresultset(stmt, tablePrivSpec, array_size(tablePrivSpec));
    HSTMT_UNLOCK(stmt);
    return ret;
}
#endif
		    ...
		    SQLCHAR *column, SQLSMALLINT columnLen)
{
    ...
    HSTMT_LOCK(stmt);
    ret = mkresultset(stmt, colPrivSpec, array_size(colPrivSpec));
    HSTMT_UNLOCK(stmt);
    return ret;
}
#endif
	      ...
	      SQLCHAR *proc, SQLSMALLINT procLen)
{
    ...
    HSTMT_LOCK(stmt);
    ret = mkresultset(stmt, procSpec, array_size(procSpec));
    HSTMT_UNLOCK(stmt);
    return ret;
}
#endif
...
SQLSetStmtOption(SQLHSTMT stmt, SQLUSMALLINT opt, SQLROWCOUNT param)
{
    ...
    HSTMT_LOCK(stmt);
    ret = drvsetstmtoption(stmt, opt, param);
    HSTMT_UNLOCK(stmt);
    return ret;
}
...
#endif
#ifdef SQL_BIGINT
	mktypeinfo(s, cc++, "bigint", SQL_BIGINT, 0);
#endif
#ifdef WINTERFACE
...
#ifdef SQL_BIGINT
	case SQL_BIGINT:
	    mktypeinfo(s, 1, "bigint", SQL_BIGINT, 28);
	    break;
#endif
#ifdef WINTERFACE
#ifdef SQL_WCHAR
...
noconn:
...
#ifdef CANT_PASS_VALIST_AS_CHARPTR
    if (s->nparams > MAX_PARAMS_FOR_MPRINTF) {
	freep(&s->query);
	...
	do {
	    s3stmt = NULL;
#if defined(HAVE_SQLITE3PREPAREV2) && HAVE_SQLITE3PREPAREV2
	    ret = sqlite3_prepare_v2(d->sqlite, sql, -1, &s3stmt, &rest);
#else
...
ConfigDlgProc(HWND hdlg, WORD wmsg, WPARAM wparam, LPARAM lparam)
{
	...
	SendDlgItemMessage(hdlg, IDC_SYNCP,
			   CB_ADDSTRING, 0, (LONG) "NORMAL");
	SendDlgItemMessage(hdlg, IDC_SYNCP,
			   CB_ADDSTRING, 0, (LONG) "OFF");
	SendDlgItemMessage(hdlg, IDC_SYNCP,
			   CB_ADDSTRING, 0, (LONG) "FULL");
	SendDlgItemMessage(hdlg, IDC_SYNCP,
		  ...
		  SQLSMALLINT *connOutLen, SQLUSMALLINT drvcompl)
{
	...
	if (connOut) {
	    if (len > 0) {
#ifdef _WIN32
		co = wmb_to_uc((SQLCHAR *) connOut, len);
#else
